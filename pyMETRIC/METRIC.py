...
L_thres = 0.001
# Minimum allowed friction velocity. Raised from upstream's 0.01 m s-1 so that
# the Monin-Obukhov stability iteration cannot drive u* arbitrarily low under
# strong stable forcing, which otherwise inflates r_ah to non-physical values
# (>1000 s m-1) and produces runaway dT calibrations.
u_friction_min = 0.1
...
def METRIC(Tr_K,
    ...
    prescribed_dT : Optional[tuple]
        If provided as (dT_a, dT_b), bypasses endmember calibration and uses
        dT = dT_a + dT_b * Tr_datum directly. cold_pixel and hot_pixel are
    ...
    flag = np.zeros(Tr_K.shape, dtype=np.uint8)
    ...
    if prescribed_dT is not None:
        # Use prescribed dT coefficients — skip endmember calibration
        dT_a, dT_b = prescribed_dT
        dT = dT_a + dT_b * Tr_datum
        print('Using prescribed dT: a=%.6f, b=%.6f' % (dT_a, dT_b))

    else:
        # Standard endmember calibration
        if cold_pixel is None or hot_pixel is None:
            raise ValueError(
                "cold_pixel and hot_pixel are required when prescribed_dT is not set")
        ...
        # Allen 2013 cold-endmember safeguard: when the cold pixel sits under
        # advective demand, LE_cold can exceed Rn-G and force H_cold strongly
        # negative, which drives the Monin-Obukhov iteration into a degenerate
        # stable corner. Cap LE_cold so that H_cold residual >= H_COLD_MIN.
        H_COLD_MIN = 5.0  # W m-2
        le_cold_cap = Rn_endmembers[0] - G_endmembers[0] - H_COLD_MIN
        if LE_endmembers[0] > le_cold_cap:
            print('Allen 2013 safeguard: capping LE_cold from %.2f to %.2f '
                  '(Rn-G=%.2f, H_cold floored at %.1f W m-2)' % (
                      float(LE_endmembers[0]), float(le_cold_cap),
                      float(Rn_endmembers[0] - G_endmembers[0]), H_COLD_MIN))
            LE_endmembers[0] = le_cold_cap
        ...
        # NOTE: test truthiness, not `is True`. Callers (PyMETRIC) pass an int
        # (1) from the config, and `1 is True` is False in Python, which would
        # silently fall through to the Kustas-Norman branch and disable the
        # METRIC/Allen resistance the config/docs intend.
        if use_METRIC_resistance:
        ...
        # dT constants (Allen 2007 eq. 29-30)
        ...
        print('Calibrated dT: a=%.6f, b=%.6f' % (dT_a, dT_b))
...
# ==============================================================================
    ...
    for n_iterations in range(max_iterations):
        ...
        if use_METRIC_resistance:
            # METRIC/Allen resistance over the current pixel subset. Broadcast
            # the fixed geometry (z_T=2 m, d_0=0, z_0H=0.1 m) to the subset
            # length; the previous np.array([2.0, 2.0]) hardcoded two elements
            # and only worked for the 2-endmember case, crashing on full images.
            R_A_params = {"z_T": np.full_like(u_friction[i], 2.0),
                          "u_friction": u_friction[i],
                          "L": L[i],
                          "d_0": np.full_like(u_friction[i], 0.0),
                          "z_0H": np.full_like(u_friction[i], 0.1)}
        else:
...
def pet_asce(T_A_K, u, ea, p, Sdn, z_u, z_T, f_cd=1, reference=TALL_REFERENCE):
    ...
    # ASCE-EWRI 2005 standardized denominator: (delta + psicr*(1 + C_d*u_2)).
    LE = (delta * (Rn - G) + psicr * C_n * u_2 * (es - ea) / T_A_K) \
         / (delta + psicr * (1.0 + C_d * u_2))
